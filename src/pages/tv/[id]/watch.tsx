import { Detail, Season } from "../../../models/type";
...
import { getTVSeasons } from "../../../api/movies";
...
const WatchTV: NextPage<WatchTVProps> = ({ seasons, data }) => {
    ...
    return (
        <>
            <Meta
                title={`${data.name} - Phim FPOLY`}
                description="Xem Các Phim Theo Các phần"
            ...
            <div className="flex justify-center">
                <div className="mt-24 md:mx-20 w-full max-w-4xl mx-6">
                    <div className="flex gap-4 flex-col md:flex-row">
                        ...
                        <div className="flex flex-col items-start gap-3 flex-grow">
                            <Link href={`/tv/${data.id}`}>
                                <a>
                                    <h1 className="text-2xl hover:text-orange-500 transition cursor-pointer">
                                        {data.name}
                                    </h1>
                                </a>
                            </Link>
                    ...
                    <h1 className="text-2xl mb-8 mt-12">Các Phần Đã Chiếu</h1>
                    {seasons.map((item) => (
                        <Fragment key={item.season_number}>
                            ...
                            >
                                ...
                                <div className="flex flex-col justify-center items-start">
                                    <h1
                                        className={`text-3xl transition ${opened === item.season_number ? "text-orange-500" : ""
                                            }`}
                                    >
                                        {item.name}
                                    </h1>
